...
CONTAINS
  ...
  SUBROUTINE ASSIGN_PTR_FLOAT(DST, SRC, START_IN_BYTES, LENGTH_IN_BYTES, SET_VALUE, SET_STREAM)
    USE ISO_C_BINDING, ONLY: C_FLOAT, C_F_POINTER, C_SIZEOF, C_SIZE_T
    IMPLICIT NONE
    ...
    INTEGER(KIND=JPIB) , INTENT(IN) :: START_IN_BYTES, LENGTH_IN_BYTES
    INTEGER(KIND=JPIB) :: END_IN_BYTES, J
    END_IN_BYTES=START_IN_BYTES + LENGTH_IN_BYTES - 1
    IF (END_IN_BYTES > SIZE(SRC, KIND=C_SIZE_T)) THEN
  ...
  SUBROUTINE ASSIGN_PTR_DOUBLE(DST, SRC, START_IN_BYTES, LENGTH_IN_BYTES, SET_VALUE, SET_STREAM)
    USE ISO_C_BINDING, ONLY: C_DOUBLE, C_F_POINTER, C_SIZEOF, C_SIZE_T
    IMPLICIT NONE
    INTEGER(KIND=C_INT8_T), POINTER, INTENT(IN) :: SRC(:)
