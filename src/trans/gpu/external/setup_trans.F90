...
IF( .NOT.D%LGRIDONLY ) THEN
  ...
  !$ACC&           COPYIN(D%NPTRLS,D%MSTABF,D%NASM0,D%OFFSETS_GEMM1,D%OFFSETS_GEMM2,D%NDGL_FS)&
  !$ACC&           COPYIN(D%NGP_OFFSET,D%NGP_A,D%NGP_B,D%NGP_STRIDE) ASYNC(1)
#else
  !$ACC ENTER DATA COPYIN(D,D%MYMS,D%NPNTGTB0,D%NPNTGTB1,D%NSTAGT0B,D%NSTAGT1B,D%NSTAGTF,D%NPROCM,D%NPROCL)&
  !$ACC&           COPYIN(D%NPTRLS,D%MSTABF,D%NASM0,D%OFFSETS_GEMM1,D%OFFSETS_GEMM2)&
  !$ACC&           COPYIN(D%NGP_OFFSET,D%NGP_A,D%NGP_B,D%NGP_STRIDE) ASYNC(1)
#endif
  ...
  !$OMP TARGET ENTER DATA MAP(TO:D%NGP_OFFSET,D%NGP_A,D%NGP_B,D%NGP_STRIDE)
