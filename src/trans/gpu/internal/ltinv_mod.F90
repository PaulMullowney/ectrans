...
MODULE LTINV_MOD
  ...
  TYPE LTINV_HANDLE
    TYPE(ALLOCATION_RESERVATION_HANDLE) :: HALL_BUFFERS
    INTEGER(KIND=C_SIZE_T) :: PIA_START
    INTEGER(KIND=C_SIZE_T) :: ZINP_START
    INTEGER(KIND=C_SIZE_T) :: ZINP0_START
    INTEGER(KIND=C_SIZE_T) :: ZOUTA_START
    INTEGER(KIND=C_SIZE_T) :: ZOUTS_START
    INTEGER(KIND=C_SIZE_T) :: ZOUTA0_START
    INTEGER(KIND=C_SIZE_T) :: ZOUTS0_START
  END TYPE
...
CONTAINS
  ...
  SUBROUTINE LTINV(ALLOCATOR,HLTINV,KF_UV,KF_SCALARS,&
    ...
    CALL ASSIGN_PTR(PIA_L, GET_ALLOCATION(ALLOCATOR, HLTINV%HALL_BUFFERS),&
        & HLTINV%PIA_START, IALLOC_SZ)
    CALL C_F_POINTER(C_LOC(PIA_L), PIA, (/ 2*IF_READIN, R%NTMAX+3, D%NUMP /))

    ! ZINP
    IALLOC_SZ = ALIGN(IIN_SIZE*SIZEOF(ZINP(1)),128)
    CALL ASSIGN_PTR(ZINP, GET_ALLOCATION(ALLOCATOR, HLTINV%HALL_BUFFERS),&
        & HLTINV%ZINP_START, IALLOC_SZ)

    ! ZINP0
    IALLOC_SZ = ALIGN(IIN0_SIZE*SIZEOF(ZINP0(1)),128)
    CALL ASSIGN_PTR(ZINP0, GET_ALLOCATION(ALLOCATOR, HLTINV%HALL_BUFFERS),&
        & HLTINV%ZINP0_START, IALLOC_SZ)

    ! ZOUTA
    IALLOC_SZ = ALIGN(IOUT_SIZE*SIZEOF(ZOUTA(1)),128)
    CALL ASSIGN_PTR(ZOUTA, GET_ALLOCATION(ALLOCATOR, HLTINV%HALL_BUFFERS),&
        & HLTINV%ZOUTA_START, IALLOC_SZ)

    ! ZOUTS
    IALLOC_SZ = ALIGN(IOUT_SIZE*SIZEOF(ZOUTS(1)),128)
    CALL ASSIGN_PTR(ZOUTS, GET_ALLOCATION(ALLOCATOR, HLTINV%HALL_BUFFERS),&
        & HLTINV%ZOUTS_START, IALLOC_SZ)

    ! ZOUTA0
    IALLOC_SZ = ALIGN(IOUT0_SIZE*SIZEOF(ZOUTA0(1)),128)
    CALL ASSIGN_PTR(ZOUTA0, GET_ALLOCATION(ALLOCATOR, HLTINV%HALL_BUFFERS),&
        & HLTINV%ZOUTA0_START, IALLOC_SZ)

    ! ZOUTS0
    IALLOC_SZ = ALIGN(IOUT0_SIZE*SIZEOF(ZOUTS0(1)),128)
    CALL ASSIGN_PTR(ZOUTS0, GET_ALLOCATION(ALLOCATOR, HLTINV%HALL_BUFFERS),&
        & HLTINV%ZOUTS0_START, IALLOC_SZ)
