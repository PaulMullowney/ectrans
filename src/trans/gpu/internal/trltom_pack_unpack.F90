...
CONTAINS
  FUNCTION PREPARE_TRLTOM_PACK(ALLOCATOR, KF_FS) RESULT(HTRLTOM_PACK)
    ...
    HTRLTOM_PACK%HFOUBUF_IN = RESERVE_GAM(ALLOCATOR, int(D%NLENGT0B*KF_FS*2*SIZEOF(DUMMY),kind=c_size_t))
...
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(G_NMEN,D_NPNTGTB0,PREEL_COMPLEX,D_NSTAGTF,D_NDGL_FS,G_NLOEN, R_NSMAX) ASYNC(1)
#endif
  ...
  SUBROUTINE TRLTOM_UNPACK(ALLOCATOR,HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
    ...
    !$ACC& PRESENT(D_NPNTGTB1,D_OFFSETS_GEMM1)

    !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(KM,ISL,IGLS,OFFSET1,OFFSET2,JGL,PAIA,PAIS) &
