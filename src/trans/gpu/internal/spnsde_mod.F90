...
! the base element of each slice; only 1:2*KF_SCALARS of it is addressed here. The second
! extent is that buffer's, which PF is read one past JI at, so it matches PIA's R%NSMAX+4.
INTEGER(KIND=JPIM), INTENT(IN)  :: KLD
REAL(KIND=JPRBT),    INTENT(IN)  :: PEPSNM(1:D%NUMP,0:R%NSMAX+2)
REAL(KIND=JPRB),    INTENT(IN)  :: PF(KLD,R%NSMAX+4,D%NUMP)
REAL(KIND=JPRB),    INTENT(OUT) :: PNSD(KLD,R%NSMAX+4,D%NUMP)
