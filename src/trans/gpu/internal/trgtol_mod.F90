...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  !     LOCAL INTEGER SCALARS
  INTEGER(KIND=JPIM) :: IFIRST, IFIRSTLAT, IGL, IGLL, ILAST,&
               &ILASTLAT, ILEN, JROC, IPOS, KPOS, INDOFF_MYPROC, IGPTROFF_JBLK, IPOS_SHIFT, ISETA, &
               &ISETB, IRECV, IRECVSET, &
               &ISETV, ISEND, ISENDSET, ITAG, J, JBLK, JFLD, &
               &JGL, JK, JL, JLOOP, ISETW,  IFLD, &
               &II,INDOFFX,IBUFLENS,IBUFLENR,INRECV, IPROC,IFLDS, &
...
#endif
  ELSE
    IF (NGPBLKS.GT.1) THEN
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(KPOS,IPOS,IFIRST,ILAST,IFLD,JK) &
      !$OMP& SHARED(INDOFF_MYPROC,NGPBLKS,IFLDS,JK_MAX,IGPTRSEND,MYSETW,INDOFF,MYPROC,IGPTROFF,IFLDOFF, &
      !$OMP&        LLUV,LLGP2,LLGP3A,LLGP3B,PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B,IUVLEVS, &
      !$OMP&        IUVPARS,IGP2PARS,IGP3ALEVS,IGP3APARS,IGP3BLEVS,IGP3BPARS)
#endif
#ifdef ACCGPU
...
#endif
    ELSE
      !WRITE(*,*) "IFLDS=",IFLDS, " JK_MAX=",JK_MAX 
      IFIRST = IGPTRSEND(1,1,MYSETW)
      ILAST = IGPTRSEND(2,1,MYSETW)
      IGPTROFF_JBLK = IGPTROFF(1)
      IPOS_SHIFT = INDOFF_MYPROC+IGPTROFF_JBLK
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) DEFAULT(NONE) PRIVATE(KPOS,IPOS,IFLD,JK) &
      !$OMP& SHARED(IFIRST,ILAST,IPOS_SHIFT,IFLDS,JK_MAX,IFLDOFF, &
      !$OMP&        LLUV,LLGP2,LLGP3A,LLGP3B,PGLAT,KINDEX,PGPUV,PGP2,PGP3A,PGP3B,IUVLEVS, &
      !$OMP&        IUVPARS,IGP2PARS,IGP3ALEVS,IGP3APARS,IGP3BLEVS,IGP3BPARS)
#endif
      DO JKL=1, JK_MAX
        DO JFLD=1,IFLDS
          ...
          IF(IFIRST > 0 .AND. JK <= ILAST) THEN
            ...
            IFLD = IFLDOFF(JFLD)
            IF(LLUV(IFLD)) THEN
              PGLAT(JFLD,KPOS) = PGPUV(JK,IUVLEVS(IFLD),IUVPARS(IFLD))
            ELSEIF(LLGP2(IFLD)) THEN
              PGLAT(JFLD,KPOS) = PGP2(JK,IGP2PARS(IFLD))
            ELSEIF(LLGP3A(IFLD)) THEN
              PGLAT(JFLD,KPOS) = PGP3A(JK,IGP3ALEVS(IFLD),IGP3APARS(IFLD))
            ELSEIF(LLGP3B(IFLD)) THEN
              PGLAT(JFLD,KPOS) = PGP3B(JK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD))
           ENDIF
          ENDIF
        ENDDO
      ENDDO
