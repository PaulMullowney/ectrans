...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  !$OMP& SHARED(NGPBLKS,IGPTRSEND,LLPGPONLY,INDOFF,MYSETW,MYPROC,IGPTROFF,IFLDS,IFLDOFF,PGLAT,KINDEX) &
  !$OMP& SHARED(PGP,LLUV,PGPUV,IUVLEVS,IUVPARS,LLGP2,PGP2,IGP2PARS,LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS,LLGP3B,PGP3B,IGP3BLEVS,IGP3BPARS) &
#endif
  DO JBLK=1,NGPBLKS
      IFIRST = IGPTRSEND(1,JBLK,MYSETW)
      IF(IFIRST > 0) THEN
  ...
  !$OMP& SHARED(INSEND,JSEND,ISEND_FLD_TOTAL,KF_GP,KVSET,NGPBLKS,IGPTRSEND,ISEND_FLD_START, &
  !$OMP& LLINDER,ZCOMBUFS,PGP,KPTRGP,LLPGPONLY,LLUV,PGPUV,IUVLEVS,IUVPARS,LLGP2,PGP2,IGP2PARS, &
  !$OMP& LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS,LLGP3B,PGP3B,IGP3BLEVS,IGP3BPARS) &
    DO INS=1,INSEND
      ISEND=JSEND(INS)
      CALL PE2SET(ISEND,ISETA,ISETB,ISETW,ISETV)
      ISENDSET = ISETV
  ...
  !$OMP& SHARED(INRECV,JRECV,IRECVTOT,ZCOMBUFR,KF_FS,KINDEX,INDOFF,PGLAT)
  DO INR=1,INRECV
      IRECV=JRECV(INR)
      ILEN = IRECVTOT(IRECV)/KF_FS
