...
#define TRGTOL_DTYPE MPI_REAL8
  ...
  SUBROUTINE TRGTOL_PACK_SEND(ZCOMBUFS,KCOMBUFS,KFLDA,KFLDA1,KFLDA2, &
    ...
    !$OMP& JI,IOFF,IPBOUND) SHARED(KFLDA,KGP_INDICES) ECTRANS_OMP_DEFAULT_CLAUSE &
    !$OMP& ECTRANS_LOOP_BOUNDS_CLAUSE(KFIELD_COUNT,KWSET_SIZE) &
    !$OMP& FIRSTPRIVATE(KNPROMA,KWSET_OFFSET,KNS,KCOMBUFS_OFFSET) &
    !$OMP& ECTRANS_DEVICE_ADDR_CLAUSE(ZCOMBUFS) &
    !$OMP& MAP(ALLOC:PGPUV,PGP2,PGP3A,PGP3B) &
    !$OMP& NOWAIT
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(PRESENT) PRIVATE(JK,JBLK,IFLD,JI,IOFF,IPBOUND) &
    !$ACC&              NO_CREATE(PGPUV,PGP2,PGP3A,PGP3B) &
    !$ACC&              FIRSTPRIVATE(KNS,KFIELD_COUNT,KWSET_SIZE,KWSET_OFFSET, &
    !$ACC&              KCOMBUFS_OFFSET,KNPROMA) ASYNC(1)
#endif
  ...
  SUBROUTINE TRGTOL_UNPACK_RECV(PREEL_REAL,KPREEL,ZCOMBUFR,KCOMBUFR, &
...
#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(IPOS) ECTRANS_OMP_DEFAULT_CLAUSE &
    ...
    !$OMP& ECTRANS_LOOP_BOUNDS_CLAUSE(KF_FS,KLEN) FIRSTPRIVATE(KGP_V,KCOMBUFR_OFFSET) &
    !$OMP& NOWAIT
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(IPOS) &
    !$ACC&              PRESENT(KNGP_A,KNGP_B,KNGP_STRIDE,PREEL_REAL,ZCOMBUFR) &
    !$ACC&              FIRSTPRIVATE(KF_FS,KLEN,KGP_V,KCOMBUFR_OFFSET) ASYNC(1)
#endif
    DO JFLD=1,KF_FS
      DO JL=1,KLEN
