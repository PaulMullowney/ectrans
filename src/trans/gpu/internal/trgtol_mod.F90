...
MODULE TRGTOL_MOD
  ...
  TYPE TRGTOL_HANDLE
     TYPE(ALLOCATION_RESERVATION_HANDLE) :: HCOMBUFS_COMBUFR_REEL
...
CONTAINS
  FUNCTION PREPARE_TRGTOL(ALLOCATOR,KF_GP,KF_FS) RESULT(HTRGTOL)
    ...
    NELEM = KF_GP*D%NGPTOT*SIZEOF(DUMMY)

    HTRGTOL%COMBUFR_START = NELEM + 1
    NELEM = NELEM + KF_FS*D%NLENGTF*SIZEOF(DUMMY) ! ZCOMBUFR

    HTRGTOL%REEL_START = NELEM + 1
    NELEM = NELEM + KF_FS*D%NLENGTF*SIZEOF(DUMMY) ! PREEL_REAL

    HTRGTOL%HCOMBUFS_COMBUFR_REEL = RESERVE(ALLOCATOR, NELEM)
...
#define TRGTOL_DTYPE MPI_DOUBLE
    ...
    block
    CALL ASSIGN_PTR(PREEL_REAL, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HCOMBUFS_COMBUFR_REEL),&
        & HTRGTOL%REEL_START, int(KF_FS*D%NLENGTF*SIZEOF(PREEL_REAL(1)),kind=c_size_t))
    end block
    ...
    IF (ISEND_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HCOMBUFS_COMBUFR_REEL),&
          & HTRGTOL%COMBUFS_START, int(ICOMBUFS_OFFSET(ISEND_COUNTS+1)*SIZEOF(ZCOMBUFS(1)),kind=c_size_t))
    ENDIF
    ...
    IF (IRECV_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HCOMBUFS_COMBUFR_REEL),&
          & HTRGTOL%COMBUFR_START, int(ICOMBUFR_OFFSET(IRECV_COUNTS+1)*SIZEOF(ZCOMBUFR(1)),kind=c_size_t))
    ENDIF
#ifdef OMPGPU
#endif
#ifdef ACCGPU
