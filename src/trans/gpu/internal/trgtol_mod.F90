...
MODULE TRGTOL_MOD
  ...
  TYPE TRGTOL_HANDLE
    ...
    TYPE(ALLOCATION_RESERVATION_HANDLE) :: HCOMBUFS, HCOMBUFR, HREEL
  END TYPE
CONTAINS
  FUNCTION PREPARE_TRGTOL(ALLOCATOR,KF_GP,KF_FS) RESULT(HTRGTOL)
    ...
    NELEM = ALIGN(int(KF_GP*D%NGPTOT*SIZEOF(DUMMY),kind=c_size_t), 128)
    HTRGTOL%HCOMBUFS = RESERVE_GAM(ALLOCATOR, NELEM)

    NELEM = ALIGN(NELEM + KF_FS*D%NLENGTF*SIZEOF(DUMMY), 128)
    HTRGTOL%HCOMBUFR = RESERVE_GAM(ALLOCATOR, NELEM)
...
#define TRGTOL_DTYPE MPI_DOUBLE
    ...
    CALL ASSIGN_PTR(PREEL_REAL, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HREEL),&
        & 1_C_SIZE_T, int(KF_FS*D%NLENGTF*SIZEOF(PREEL_REAL(1)),kind=c_size_t))
    ...
    IF (ISEND_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION_GAM(ALLOCATOR, HTRGTOL%HCOMBUFS),&
...
#endif
#ifdef ACCGPU
        !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(JK,JBLK,IFLD,JI) &
...
#endif
#ifdef ACCGPU
        !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(JK,JBLK,IFLD,JI,IOFF,PBOUND) &
        !$ACC&              FIRSTPRIVATE(ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,&
        !$ACC&              ICOMBUFS_OFFSET_V,NPROMA) ASYNC(1)
#endif
        DO JFLD=1,ISEND_FIELD_COUNT_V
    ...
    IF (IRECV_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION_GAM(ALLOCATOR, HTRGTOL%HCOMBUFR),&
           & 1_C_SIZE_T, &
           & int(ICOMBUFR_OFFSET(IRECV_COUNTS+1)*SIZEOF(ZCOMBUFR(1)),kind=c_size_t))
    ENDIF
#ifdef OMPGPU
#endif
#ifdef ACCGPU
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(II,IPOS) FIRSTPRIVATE(KF_FS,ILEN, &
