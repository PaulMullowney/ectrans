...
CONTAINS
  FUNCTION PREPARE_TRLTOG(ALLOCATOR,KF_FS,KF_GP) RESULT(HTRLTOG)
    ...
    HTRLTOG%HCOMBUFR = RESERVE_GAM(ALLOCATOR, NELEM)
    NELEM = ALIGN(KF_FS*D%NLENGTF*SIZEOF(DUMMY),128) !ZCOMBUFS upper obund
    HTRLTOG%HCOMBUFS = RESERVE_GAM(ALLOCATOR, NELEM)
...
#define TRLTOG_DTYPE MPI_DOUBLE
    ...
    IF (IRECV_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION_GAM(ALLOCATOR, HTRLTOG%HCOMBUFR),&
          & 1_C_SIZE_T, int(ICOMBUFR_OFFSET(IRECV_COUNTS+1)*SIZEOF(ZCOMBUFR(1)),kind=c_size_t))
    ENDIF
    IF (ISEND_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION_GAM(ALLOCATOR, HTRLTOG%HCOMBUFS),&
