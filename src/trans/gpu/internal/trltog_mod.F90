...
CONTAINS
  ...
  SUBROUTINE TRLTOG(ALLOCATOR,HTRLTOG,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,KPTRGP,&
    ...
    REAL(KIND=JPRB),OPTIONAL,INTENT(OUT),CONTIGUOUS,TARGET :: PGPUV(:,:,:,:)
    REAL(KIND=JPRB),OPTIONAL,INTENT(OUT),CONTIGUOUS,TARGET :: PGP3A(:,:,:,:)
    REAL(KIND=JPRB),OPTIONAL,INTENT(OUT),CONTIGUOUS,TARGET :: PGP3B(:,:,:,:)
    REAL(KIND=JPRB),OPTIONAL,INTENT(OUT),CONTIGUOUS,TARGET :: PGP2(:,:,:)
#else
    ...
    INTEGER(KIND=JPIM) :: JFLD, J, JI, JK, JL, IFLDS, JROC, INR, INS
    ...
    ! reaches the explicit-shape arguments of TRLTOG_UNPACK_RECV.
...
#define TRLTOG_DTYPE MPI_REAL8
    ...
    ! of ECTRANS_MAP_PRESENT_ALLOC can be used here. The D%NGP_* tables are mapped once in
    ! SETUP_TRANS, so they need no region either.
#endif
#ifdef ACCGPU
