...
! CONTIGUOUS so that the PSPSC3A(:,:,J3) sections handed to UPDSPB are simply contiguous, and so
! satisfy its CONTIGUOUS dummy without a packing temporary that would not be the storage the
! caller mapped MAP(FROM).
