...
!$OMP TARGET DATA &
!$OMP& MAP(PRESENT,ALLOC:D_NPTRLS,D_NSTAGTF,F_RACTHE,G_NMEN,G_NLOEN,R_NSMAX)
#endif
#ifdef ACCGPU
!$ACC DATA &
...
!$OMP& FIRSTPRIVATE(IBEG,IEND,IINC,KF_UV,OFFSET_VAR,KF_FS,KUV_OFFSET)
#endif
#ifdef ACCGPU
...
IF (KSCALARS_NSDER_OFFSET >= 0) THEN
  ...
  !$OMP& SHARED(PREEL_COMPLEX) &
  !$OMP& FIRSTPRIVATE(IBEG,IEND,IINC,KF_SCALARS,OFFSET_VAR,KF_FS,KSCALARS_NSDER_OFFSET)
#endif
#ifdef ACCGPU
...
IF (KUV_EWDER_OFFSET >= 0) THEN
  ...
  !$OMP& SHARED(PREEL_COMPLEX) &
  !$OMP& FIRSTPRIVATE(IBEG,IEND,IINC,KF_UV,ILOEN_MAX,OFFSET_VAR,KF_FS,KUV_OFFSET,&
  !$OMP&              KUV_EWDER_OFFSET)
#endif
#ifdef ACCGPU
...
IF (KSCALARS_EWDER_OFFSET > 0) THEN
  ...
  !$OMP& SHARED(PREEL_COMPLEX) &
  !$OMP& FIRSTPRIVATE(IBEG,IEND,IINC,KF_SCALARS,ILOEN_MAX,OFFSET_VAR,KF_FS,&
  !$OMP&              KSCALARS_EWDER_OFFSET,KSCALARS_OFFSET)
#endif
#ifdef ACCGPU
