...
INTEGER(KIND=JPIM) :: JROC,ISETA,ISETB,ISETW,ISETV,IFIRSTLAT,ILASTLAT,IGLL,JL
INTEGER(KIND=JPIM) :: IGPIDX
...
! Gridpoint index table shared by TRLTOG and TRGTOL. See DISTR_TYPE for the layout.
IF (.NOT. D%LGRIDONLY) THEN
  ALLOCATE(D%NGP_OFFSET(NPROC+1))

  ! Size the table: the points this task exchanges with every task, in send/recv order.
  IGPIDX = 0
  DO JROC=1,NPROC
    ...
    DO JGL=IFIRSTLAT,ILASTLAT
      ...
      IGPIDX = IGPIDX+D%NONL(IGL,ISETB)
    ENDDO
  ENDDO

  ALLOCATE(D%NGP_A(IGPIDX))
  ALLOCATE(D%NGP_B(IGPIDX))
  ALLOCATE(D%NGP_STRIDE(IGPIDX))

  IGPIDX = 0
  DO JROC=1,NPROC
    D%NGP_OFFSET(JROC) = IGPIDX
    CALL PE2SET(JROC,ISETA,ISETB,ISETW,ISETV)
    ! Overlap between what the sender holds (its A set) and what this task holds (its W set).
    IFIRSTLAT = MAX(D%NPTRLS(MYSETW),D%NFRSTLAT(ISETA))
    ILASTLAT  = MIN(D%NPTRLS(MYSETW)+D%NULTPP(MYSETW)-1,D%NLSTLAT(ISETA))
    DO JGL=IFIRSTLAT,ILASTLAT
      ...
      DO JL=1,D%NONL(IGL,ISETB)
        IGPIDX = IGPIDX+1
        ! offset to first layer of this gridpoint, less the factor of KF_FS
        D%NGP_A(IGPIDX)      = INT(D%NSTAGTF(IGLL),JPIM)
        D%NGP_B(IGPIDX)      = (D%NSTA(IGL,ISETB)-1)+(JL-1)
        ! distance between two layers of this gridpoint
        D%NGP_STRIDE(IGPIDX) = INT(D%NSTAGTF(IGLL+1)-D%NSTAGTF(IGLL),JPIM)
      ENDDO
    ENDDO
  ENDDO
  D%NGP_OFFSET(NPROC+1) = IGPIDX
ENDIF
